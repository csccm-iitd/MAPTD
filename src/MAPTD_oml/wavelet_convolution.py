...
class WaveConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_coeff = [torch.zeros(x.shape[0], self.out_channels, coeffs.shape[-1], device= x.device) for coeffs in x_coeff]
...
class WaveConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Instantiate higher level coefficients as zeros
        out_ft = torch.zeros_like(x_ft, device= x.device)
        out_coeff = [torch.zeros(x.shape[0], self.out_channels, 3, coeffs.shape[-2], coeffs.shape[-1], device= x.device) for coeffs in x_coeff]
