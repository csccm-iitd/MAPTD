...
PATH = directory + '/MPC/results/beam_mpc2.mat'
ntrain = 490
ntest = 100
...
for ep in range(epochs):
    ...
    for xx, fin, uin, yy in train_loader:
        ...
        loss = 5*myloss(out[...,0].reshape(yy.shape[0], -1), yy[...,0].reshape(yy.shape[0], -1)) + \
                myloss(out[...,1].reshape(yy.shape[0], -1), yy[...,1].reshape(yy.shape[0], -1)) + \
                myloss(out[...,2].reshape(yy.shape[0], -1), yy[...,2].reshape(yy.shape[0], -1))
...
samples = np.random.choice(ntest,3)
...
"""
torch.save(model, 'model/WNO_beam_control2')
scipy.io.savemat('results/wno_results_beam_control2.mat', mdict={'x_test':x_test.cpu().numpy(),
