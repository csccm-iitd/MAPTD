"""
The MAPTD algorithm with pretrained environment model
...
class DRL(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        weights = torch.load(cfg.trained_dynamics)
        new_state_dict = {}
        for key, value in weights.items():
            if key.startswith('net.'):
                new_key = key[4:]  # Remove the 'net.' prefix (first 4 characters)
                new_state_dict[new_key] = value
            else:
                new_state_dict[key] = value # Keep keys that don't have the prefix

        self._dynamics.load_state_dict(new_state_dict)
        self._dynamics.eval()
