...
__CONFIG__, __LOGS__, task_name = 'cfgs', 'logs', 'beam_offline'
...
print("Save Directory: ", work_dir)
env, agent, buffer = make_env(cfg), MAPTD_nn(cfg), ReplayBuffer(cfg)

agent.load('logs/beam-EulerBeam/offline/1/models/model.pt')
...
# Save the results,
# -------------------------------------------------
sp.io.savemat('results/MAPTD_NN_beam_offline.mat', mdict={'xt_controlled':disp,
                                                  'xt_uncontrolled':true_disp,
                                                  'rewards':rewards,
                                                  'actions':actions})
sp.io.savemat('results/time_beam_offline.mat', mdict={'time':time}) 
